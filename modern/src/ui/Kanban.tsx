...
export function Kanban({ contacts, stages, onSelect, onMove, labelNames }: Props) {
  ...
  return <section className="panel funnel">
    ...
    <div className="kanban">
      {columns.map(({ stage, items }) => <KanbanColumn key={stage.id} stage={stage} items={items} stages={ordered} onSelect={onSelect} onMove={onMove} labelNames={labelNames} />)}
      {unassigned.length > 0 && <KanbanColumn stage={{ id: '', name: 'Sem estágio', position: 999 }} items={unassigned} stages={ordered} onSelect={onSelect} onMove={onMove} labelNames={labelNames} unassigned />}
    </div>
  </section>;
}
...
function KanbanColumn({
  ...
  return <div className={`kanban-column${unassigned ? ' unassigned' : ''}`} onDragOver={(e) => e.preventDefault()} onDrop={(e) => { const id = e.dataTransfer.getData('text/plain'); if (id && stage.id) onMove(id, stage.id); }}>
    ...
    <div className="kanban-cards">{items.map((contact) => <article className="kanban-card" draggable key={contact.id} onDragStart={(e) => e.dataTransfer.setData('text/plain', contact.id)} onClick={() => onSelect(contact.id)}>
      ...
      {contact.labels.length > 0 && <div className="tags">{contact.labels.slice(0, 3).map((label) => <span className="tag" key={label}>{labelNames?.get(label) ?? label}</span>)}</div>}
